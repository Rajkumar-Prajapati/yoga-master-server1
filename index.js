...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASSWORD}@cluster0.fksvvmp.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0`;
...
async function run() {
  try {
    ...
    // GET ALL CLASSES ADDED BY INSTRUCTOR BY EMAIL of the instructor
    app.get(
      "/classes/:email",
      verifyJWT,
      verifyInstructor,
    ...
    // GET ALL CLASSES with the status of approved only
    ...
    app.post("/create-payment-intent", verifyJWT, async (req, res) => {
      ...
      const paymentIntent = await stripe.paymentIntents.create({
        ...
        description: "for yoga master payment",
      });
      res.send({
